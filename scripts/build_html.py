...
.tree-node:hover .node-card {
  /* Gold halo on dark — filter only (no scale: SVG scale shifts hit-area causing hover flicker) */
  filter: drop-shadow(0 0 10px rgba(212, 164, 88, 0.55)) drop-shadow(0 4px 18px rgba(0,0,0,0.7));
}
...
// Node card dimensions — compact single-card layout
const NW = 220;   // node width
const NH = 76;    // base card height (no spouse band)
const NH_SP = 90; // card height when spouse band is present
const SPOUSE_BAND_H = 22; // height of spouse band at bottom of card
const SPINE = 68; // left margin for generation labels
const M = { top: 56, right: 80, bottom: 72, left: SPINE + 24 };

// Node spacing — single card only, compact
const NODE_DX = 252;   // horizontal breathing room between branches
const NODE_DY = 120;   // vertical: card height + gap between levels
...
// Helper: effective card height for a datum (taller if spouse band)
function cardH(d) {
  return (d.data && d.data.spouse && d.data.spouse.trim()) ? NH_SP : NH;
}

// Helper: extract just the spouse name (stop before parenthesis or year)
function parseSpouseName(spouseStr) {
  if (!spouseStr) return null;
  const m = spouseStr.match(/^([^(,\d]+)/);
  if (m) return m[1].trim();
  return spouseStr.split(/[,(]/)[0].trim();
}
...
root.each(d => {
  ...
  if (d.y - ch/2 < minY) minY = d.y - ch/2;
  if (d.y + ch/2 > maxY) maxY = d.y + ch/2;
});
...
g.append("g")
  ...
  .attr("d", d => {
    const sx = d.source.x, sy = d.source.y + cardH(d.source)/2;
    const tx = d.target.x, ty = d.target.y - cardH(d.target)/2;
    const midY = (sy + ty) / 2;
...
cardG.append("rect")
  .attr("x", -NW/2 + 3).attr("y", d => -cardH(d)/2 + 4)
  .attr("width", NW).attr("height", d => cardH(d))
  .attr("rx", 7).attr("ry", 7)
  .attr("fill", "rgba(0,0,0,0.5)");

// Main card rect with gradient fill
cardG.append("rect")
  .attr("x", -NW/2).attr("y", d => -cardH(d)/2)
  .attr("width", NW).attr("height", d => cardH(d))
  .attr("rx", 7).attr("ry", 7)
...
cardG.append("rect")
  .attr("x", -NW/2 + 2).attr("y", d => -cardH(d)/2 + 1)
...
nodeGroups.filter(d => d.data.immigrant_to_america)
  .each(function(d) {
    ...
    sg.append('rect')
      .attr('x', -NW/2 - 4).attr('y', -ch/2 - 4)
      .attr('width', NW + 8).attr('height', ch + 8)
    ...
    sg.append('rect')
      .attr('x', -NW/2 + 0.5).attr('y', -ch/2 + 0.5)
      .attr('width', NW - 1).attr('height', ch - 1)
      .attr('rx', 7).attr('ry', 7)
      .attr('fill', 'none')
      .attr('stroke', '#16a34a')
    ...
    if (d.data.country_flag) {
      sg.append('text')
        .attr('x', NW/2 - 14).attr('y', -ch/2 + 16)
        .attr('font-size', 17)
        .attr('text-anchor', 'middle')
        .attr('pointer-events', 'none')
        .text(d.data.country_flag);
    }
    // "★ IMMIGRANT" small label bottom-left (above spouse band if present)
    sg.append('text')
      .attr('x', -NW/2 + 6).attr('y', d.data.spouse ? ch/2 - SPOUSE_BAND_H - 4 : ch/2 - 6)
      .attr('font-size', 8)
...
nodeGroups.filter(d => d.data.confidence === 'unverified')
  .each(function(d) {
    ...
    sg.append('rect')
      ...
      .attr('pointer-events', 'none');
    sg.append('rect')
      .attr('x', -NW/2).attr('y', -ch/2 + 10)
      .attr('width', NW).attr('height', 8)
      .attr('fill', 'rgba(180,60,40,0.32)')
      .attr('pointer-events', 'none');

    sg.append('text')
      .attr('x', 0).attr('y', -ch/2 + 13)
      .attr('text-anchor', 'middle')
    ...
    // Large "?" bottom-right — 34px serif, gold glow (stays in person area, above spouse band)
    const qY = d.data.spouse ? ch/2 - SPOUSE_BAND_H - 5 : ch/2 - 5;
    sg.append('text')
      .attr('x', NW/2 - 16).attr('y', qY)
      .attr('text-anchor', 'middle')
      .attr('font-family', "'Cormorant Garamond', Georgia, serif")
      .attr('font-size', '34px')
      .attr('font-weight', '700')
...
nodeGroups.filter(d => d.data.confidence === 'possible')
  .each(function(d) {
    ...
    const qY = d.data.spouse ? ch/2 - SPOUSE_BAND_H - 4 : ch/2 - 4;
    sg.append('text')
      .attr('x', NW/2 - 16).attr('y', qY)
      .attr('text-anchor', 'middle')
      .attr('font-family', "'Cormorant Garamond', Georgia, serif")
...
// ── Node text ────────────────────────────────────────────────────
// Text is centered in the person area (above spouse band if present).
// Text shifts right when a portrait is present on the left.

// Name line — anchored relative to card top
cardG.append("text")
  .attr("x", d => d.data.portrait_url ? (TEXT_X_PORT - NW/2 + PORT_R) : 0)
  .attr("y", d => -cardH(d)/2 + 26)
  ...
  .text(d => truncate(d.data.name, d.data.portrait_url ? 21 : 30));

// Dates — monospaced
cardG.append("text")
  .attr("x", d => d.data.portrait_url ? (TEXT_X_PORT - NW/2 + PORT_R) : 0)
  .attr("y", d => -cardH(d)/2 + 41)
  .attr("text-anchor", d => d.data.portrait_url ? "start" : "middle")
...
// ── Spouse band — rendered inline at bottom of card ──────────────
nodeGroups.filter(d => d.data.spouse && d.data.spouse.trim())
  .each(function(d) {
    const sg = d3.select(this).select('.node-card');
    const ch = cardH(d); // NH_SP
    const bandY = ch/2 - SPOUSE_BAND_H;

    // Separator line above band
    sg.append('line')
      .attr('x1', -NW/2 + 8).attr('y1', bandY)
      .attr('x2', NW/2 - 8).attr('y2', bandY)
      .attr('stroke', 'rgba(212,164,88,0.25)')
      .attr('stroke-width', 0.6)
      .attr('pointer-events', 'none');

    // Band background — subtly lighter than card
    sg.append('rect')
      .attr('x', -NW/2).attr('y', bandY)
      .attr('width', NW).attr('height', SPOUSE_BAND_H)
      .attr('rx', 0).attr('ry', 0)
      .attr('fill', 'rgba(255,255,255,0.06)')
      .attr('pointer-events', 'none');
    // Round bottom corners only (extra rect to fake bottom-radius)
    sg.append('rect')
      .attr('x', -NW/2).attr('y', ch/2 - 7)
      .attr('width', NW).attr('height', 7)
      .attr('rx', 7).attr('ry', 7)
      .attr('fill', 'rgba(255,255,255,0.06)')
      .attr('pointer-events', 'none');

    // "♥ m. Spouse Name" italic text
    const spouseName = parseSpouseName(d.data.spouse) || d.data.spouse;
    sg.append('text')
      .attr('x', 0).attr('y', bandY + 15)
      .attr('text-anchor', 'middle')
      .attr('font-family', "'Lora', Georgia, serif")
      .attr('font-style', 'italic')
      .attr('font-size', '9.5px')
      .attr('fill', 'rgba(212,164,88,0.75)')
      .attr('pointer-events', 'none')
      .text('♥ m. ' + truncate(spouseName, 26));
  });

// ── Generation badge (top-left diamond) ─────────────────────────
const badgeX = -NW/2 + 11;

// Diamond shape — gold on dark (data-driven so it uses correct card height)
cardG.append("path")
  .attr("d", d => {
    const by = -cardH(d)/2 + 11;
    return `M${badgeX},${by - 9} L${badgeX + 9},${by} L${badgeX},${by + 9} L${badgeX - 9},${by} Z`;
  })
...
cardG.append("text")
  .attr("x", badgeX).attr("y", d => -cardH(d)/2 + 14.5)
  .attr("text-anchor", "middle")
  .attr("font-family", "'Cormorant Garamond', Georgia, serif")
  .attr("font-size", "9px")
  .attr("font-weight", "600")
...
(function renderSecondaryLineageTrees() {
  ...
  const controllers = [];
  const CARD_W = 220;
  const CARD_H = 76;       // base height (no spouse)
  const CARD_H_SP = 90;    // height with spouse band
  const SPOUSE_BAND = 22;  // spouse band height
  function cardH2(d) {
    return (d.data && d.data.spouse && d.data.spouse.trim()) ? CARD_H_SP : CARD_H;
  }
  function parseSpouseName2(s) {
    if (!s) return null;
    const m = s.match(/^([^(,\d]+)/);
    if (m) return m[1].trim();
  ...
  function renderLine(line, index) {
    ...
    root.each(d => {
      ...
      if (d.y - ch/2 < mnY) mnY = d.y - ch/2;
      if (d.y + ch/2 > mxY) mxY = d.y + ch/2;
    });
    ...
    g.append('g')
      ...
      .attr('d', d => {
        const sx = d.source.x;
        const sy = d.source.y + cardH2(d.source) / 2;
        const tx = d.target.x;
        const ty = d.target.y - cardH2(d.target) / 2;
        const midY = (sy + ty) / 2;
        return `M${sx},${sy} C${sx},${midY} ${tx},${midY} ${tx},${ty}`;
      });
    ...
    card.append('rect')
      .attr('x', -CARD_W / 2 + 3)
      .attr('y', d => -cardH2(d) / 2 + 4)
      .attr('width', CARD_W)
      .attr('height', d => cardH2(d))
      .attr('rx', 7).attr('ry', 7)
    ...
    nodeGroups.filter(d => d.data.immigrant_to_america)
      .each(function(d) {
        ...
        const ch = cardH2(d);
        sg.append('rect')
          .attr('x', -CARD_W/2 - 4).attr('y', -ch/2 - 4)
          .attr('width', CARD_W + 8).attr('height', ch + 8)
          .attr('rx', 10).attr('ry', 10)
          .attr('fill', 'none')
          .attr('stroke', '#22c55e')
          .attr('stroke-width', 5)
          ...
          .attr('pointer-events', 'none');
        sg.append('rect')
          .attr('x', -CARD_W/2 + 0.5).attr('y', -ch/2 + 0.5)
          .attr('width', CARD_W - 1).attr('height', ch - 1)
          .attr('rx', 7).attr('ry', 7)
          .attr('fill', 'none')
          .attr('stroke', '#16a34a')
          .attr('stroke-width', 2.5)
        ...
        if (d.data.country_flag) {
          sg.append('text')
            .attr('x', CARD_W/2 - 13).attr('y', -ch/2 + 14)
            .attr('font-size', 14)
            .attr('text-anchor', 'middle')
            .attr('pointer-events', 'none')
            .text(d.data.country_flag);
        ...
        sg.append('text')
          .attr('x', -CARD_W/2 + 6).attr('y', d.data.spouse ? ch/2 - SPOUSE_BAND - 4 : ch/2 - 5)
          .attr('font-size', 7.5)
          .attr('font-weight', 700)
          .attr('letter-spacing', '0.1em')
          .attr('fill', '#22c55e')
    ...
    nodeGroups.filter(d => d.data.is_notable)
      .each(function(d) {
        ...
        const ch = cardH2(d);
        sg.append('text')
          .attr('x', -CARD_W/2 + 14).attr('y', -ch/2 + 14)
          .attr('font-size', 12)
          .attr('text-anchor', 'middle')
          .attr('pointer-events', 'none')
          .text('⭐');
    ...
    card.filter(d => ['possible', 'unverified'].includes(normalizeConfidence(d.data.confidence)))
      ...
      .attr('y', d => {
        const ch = cardH2(d);
        return d.data.spouse ? ch/2 - SPOUSE_BAND - 5 : ch/2 - 5;
      })
      .attr('text-anchor', 'middle')
      .attr('font-family', "'Cormorant Garamond', Georgia, serif")
    ...
    const badgeX2 = -CARD_W / 2 + 12;
    card.append('path')
      .attr('d', d => {
        const by = -cardH2(d) / 2 + 12;
        return `M${badgeX2},${by - 9} L${badgeX2 + 9},${by} L${badgeX2},${by + 9} L${badgeX2 - 9},${by} Z`;
      })
...
');
    ...
    // ── Spouse band — inline at bottom of card ─────────────────
    nodeGroups.filter(d => d.data.spouse && d.data.spouse.trim())
      .each(function(d) {
        const sg = d3.select(this).select('.node-card');
        const ch = cardH2(d);
        const bandY = ch/2 - SPOUSE_BAND;

        sg.append('line')
          .attr('x1', -CARD_W/2 + 8).attr('y1', bandY)
          .attr('x2', CARD_W/2 - 8).attr('y2', bandY)
          .attr('stroke', 'rgba(212,164,88,0.25)')
          .attr('stroke-width', 0.6)
          .attr('pointer-events', 'none');

        sg.append('rect')
          .attr('x', -CARD_W/2).attr('y', bandY)
          .attr('width', CARD_W).attr('height', SPOUSE_BAND)
          .attr('fill', 'rgba(255,255,255,0.06)')
          .attr('pointer-events', 'none');
        sg.append('rect')
          .attr('x', -CARD_W/2).attr('y', ch/2 - 7)
          .attr('width', CARD_W).attr('height', 7)
          .attr('rx', 7).attr('ry', 7)
          .attr('fill', 'rgba(255,255,255,0.06)')
          .attr('pointer-events', 'none');

        const spouseName = parseSpouseName2(d.data.spouse) || d.data.spouse;
        sg.append('text')
          .attr('x', 0).attr('y', bandY + 15)
          .attr('text-anchor', 'middle')
          .attr('font-family', "'Lora', Georgia, serif")
